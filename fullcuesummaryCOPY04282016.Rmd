...
setwd("~/Dropbox/BU2016/FBA Workspace")
library(knitr)

#unconstrained CUE
library(vegan)
dlsum=read.csv(file="Model Downloading Summaries/modeldownloadsummary.csv")
  rownames(dlsum)=dlsum[,1]
  dlsum=dlsum[,2:5]
cue=read.csv(file="Unconstrained CUE/CUESummary02292016.csv") #CUE calculations
nmds=read.csv(file="Exchange Reaction Database/nmdsmatrix.csv") #reaction presence/absence
exch=read.csv(file="Exchange Reaction Database/exchangecounts.csv")
biomasscomp=read.csv(file="Unconstrained CUE/gnegbiomasstable.csv")
exchaccumulation=read.csv(file="Exchange Reaction Database/exchangeaccumulation.csv")
...
require(phytools) 
require(lsr)
tips=read.csv(file="multiphylosignal/tiplabels04252016.csv")
  newtips=as.vector(tips$newtiplabel)
cuevals=read.csv(file="multiphylosignal/treedata04252016.csv",row.names=1)
tipcolors=read.csv(file="multiphylosignal/tipcolors.csv",header=TRUE)
  rownames(tipcolors)=tipcolors[,1]
  tipcolors=tipcolors[,-1]

  #droptips=list(as.vector(read.csv(file="multiphylosignal/droplist04252016.csv"))) 
...
#kinetics
con=read.csv(file="Constraining and Essential/constraineddifferences.csv")
  contrim=con
  contrim[contrim==0]<-NA
kinetics=read.csv(file="constraininguptakekinetics.csv")
kinsum=read.csv(file="Constraining and Essential/kineticsummary.csv")
library(drc)
```
...
Unconstrained CUE ranged from `r min(cuevals[,3])` to `r max(cuevals[,3])`.
...
```{r,echo=FALSE}
cuetreenex=read.nexus(file="multiphylosignal/CUETREE4192016")
...
plot(droproottree,show.tip.label=T,show.node.label = FALSE,cex=0.3, main="Tree Structure",align.tip.label=T, tip.color=as.matrix(tipcolors[,7]))
legend("bottomleft",legend=c(rownames(dlsum)),fill=c(as.matrix(dlsum[,4])),cex=0.4,ncol=1,bty="n")
...
for (i in (1:6)){
        ...
        cuecontmap=contMap(droproottree,traitvec,plot=T,fsize=c(0.25),type="fan")
...
allcon=read.csv(file="Constraining and Essential/allconstrainedsummary04252016.csv",row.names=1,header=T)
